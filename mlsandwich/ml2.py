...
sim_data = pd.read_csv("./data/simulated_base2.csv", index_col=["run", "tick"]).query('tick > 310').sort_index()
runs = list(range(0, 1000))
...
X_train_reshaped = X_train_scaled.reshape(-1, 55, 1)
X_test_reshaped = X_test_scaled.reshape(-1, 55, 1)
...
def create_model(dense_neurons=32, learning_rate=0.01, filters=32, kernel_size=3):
    model = tf.keras.Sequential([
        tf.keras.layers.Conv1D(filters=filters, kernel_size=kernel_size, activation='relu', input_shape=(55, 1)),
